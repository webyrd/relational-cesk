...
(import (rnrs)
        ...
        (test-check-lib)
        (cesk-quines)
        (rename (cesk-quines-simple) (evalo evalo-simple) (eval-expo eval-expo-simple) (empty-env empty-env-simple) (empty-store empty-store-simple) (empty-k empty-k-simple)))
    ...
    (fresh (expr env store k k^ val)
      ...
      (condu
        [(eval-expo expr env store k^ val)
         (== `(,expr ,env ,store ,k ,val) q)]
        [(project (expr env store k val)
           (begin
             (errorf 'combined-test-1 "eval-expo can't handle state generated by eval-expo-simpl: ~s ~s ~s ~s\n" env store k val)
             (== #f #f)))]))))
  50)
